...
class VideoAnalyzer:
    ...
    def analyze_frame(self, frame, frame_idx):
        ...
        # 2. BRIGHTNESS THRESHOLD (Value Channel)
        # We target the glowing part of the flame (Value > 180)
        _, v_mask = cv2.threshold(hsv[:, :, 2], self.flame_threshold, 255, cv2.THRESH_BINARY)

        # 3. COLOR THRESHOLD (Hue/Saturation)
        # Red/Yellow/White/Light-Blue typical flame colors
        color_mask = cv2.inRange(hsv, self.lower_flame, self.upper_flame)

        # 4. HYBRID MASK
        mask = cv2.bitwise_and(v_mask, color_mask)

        # Physical Cutoff: Zero out anything at or below the nozzle
        ...
        if contours:
            ...
            if cv2.contourArea(c) > 15:  # Filter out tiny noise
                x_start, y_start, w_c, h_c = cv2.boundingRect(c)

                # Scan columns to find the highest pixel (top of the flame)
                # or lowest pixel if calculating base. Here we find "average bottom" of visible flame.
                bottom_y_values = []
                for x_col in range(x_start, x_start + w_c):
                    col_pixels = mask[:, x_col]
                    hits = np.where(col_pixels == 255)[0]
                    if len(hits) > 0:
                        bottom_y_values.append(hits[-1])

                if bottom_y_values:
                    avg_bottom_y = np.mean(bottom_y_values)
                    dist_px = max(0, np.subtract(self.nozzle_y, avg_bottom_y))
                    dist_cm = dist_px / self.scale_ratio
                    dist_std = np.std(bottom_y_values) / self.scale_ratio

                    cv2.drawContours(roi_frame, [c], -1, (0, 255, 0), 1)
                    cv2.line(roi_frame, (x_start, int(avg_bottom_y)), (x_start + w_c, int(avg_bottom_y)), (255, 0, 0),
                             1)
    ...
    def run(self):
        ...
        out = cv2.VideoWriter(str(self.video_out_path), cv2.VideoWriter_fourcc(*'mp4v'), 30.0, (vw, total_h))
